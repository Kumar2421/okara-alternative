...
export async function POST(req: NextRequest) {
  ...
  if (FEATURES.PLATFORM_MODE) {
    ...
    // jsonb column — Supabase already returns a parsed object, unlike the
    // SQLite TEXT column which needs JSON.parse().
    ...
    if (conn?.api_key_secret_id) {
      ...
      // free in platform mode for now — no credit_costs entry yet for product-info
    ...
    try {
      const generator = new ProductInfoGenerator(driver, apiKey, baseUrl);
